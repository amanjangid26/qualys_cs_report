...
# Streaming: images are read from page files one at a time.
# =============================================================================
CSV_HEADERS = [
    # Image columns (16)
...
# Instead of loading ALL images into one giant list (1.7 GB for 10K images),
# we iterate through saved page files ONE AT A TIME (250 images per page).
# Memory stays constant (~45 MB) regardless of total image count.
# =============================================================================

def iterate_images_from_pages(pages_directory, label):
    """Yield images one at a time by reading page files sequentially.
    Each page is loaded, yielded from, then discarded from memory."""
    page = 1
    while True:
        page_file = os.path.join(pages_directory, f"{label}_{page:04d}.json")
...
def collect_shas_from_pages(pages_directory, label):
    """Collect unique SHAs from page files. Only stores SHA strings, not full images."""
    shas = set()
...
def generate_csv_report_streaming(pages_directory, label, eol_sha_set,
                                   container_counts, skip_containers,
                                   output_path, logger):
    """Write CSV by streaming through page files one at a time.

    Memory usage: ~1 page (250 images × ~175 KB = ~45 MB) at any point.
    This is CONSTANT regardless of total image count.

    Old approach: load all images → enrich all → write all (3 GB for 10K images)
    New approach: load page → enrich page → write page → discard → next page (~45 MB always)
    """
    ...
    with open(temp_path, "w", newline="", encoding="utf-8") as csv_file:
        ...
        # Stream through images one page at a time
        for image in iterate_images_from_pages(pages_directory, label):
            total_images += 1
            ...
            # EOL: only evaluate when OS is present (not for distroless)
            if operating_system:
                eol_base_os = image_sha in eol_sha_set
            else:
                eol_base_os = ""
            if eol_base_os is True:
                total_eol += 1

            container_count = container_counts.get(image_sha, "N/A") if not skip_containers else "N/A"

            # Build image-level columns (shared across all repos for this image)
            repos = extract_image_repositories(image)
            image_base = [
                convert_to_safe_string(image.get("imageId")),
                image_sha,
                operating_system,
                eol_base_os,
                convert_to_safe_string(image.get("architecture")),
                convert_epoch_ms_to_iso(image.get("created")),
                convert_epoch_ms_to_iso(image.get("lastScanned")),
                " | ".join(convert_to_safe_string(s) for s in (image.get("scanTypes") or [])),
                " | ".join(convert_to_safe_string(s) for s in (image.get("source") or [])),
            ]

            for repo in repos:
                image_cols = image_base + [
                    repo["registry"], repo["repository"], repo["tag"],
                    convert_to_safe_string(image.get("riskScore")),
                    convert_to_safe_string(image.get("maxQdsScore")),
                    convert_to_safe_string(image.get("qdsSeverity")),
                    container_count,
                ]
    ...
    return total_rows, total_images, total_eol


def generate_json_report_streaming(pages_directory, label, eol_sha_set,
                                    container_counts, skip_containers,
                                    output_path, logger):
    """Write JSON report by streaming. Writes images one at a time to avoid
    ...
    with open(temp_path, "w", encoding="utf-8") as f:
        # Write JSON structure manually to avoid holding all images in memory
        f.write('{\n')
        f.write(f'  "generatedAt": "{datetime.now(tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")}",\n')
        f.write('  "images": [\n')
        ...
        for image in iterate_images_from_pages(pages_directory, label):
            total_images += 1
            image_sha = image.get("sha", "")
            os_val = convert_to_safe_string(image.get("operatingSystem"))
            eol_val = (image_sha in eol_sha_set) if os_val else ""
            if eol_val is True:
                total_eol += 1

            # Build a lightweight summary (not the full raw API response)
            summary = {
                "imageId": convert_to_safe_string(image.get("imageId")),
                "sha": image_sha,
                "operatingSystem": os_val,
                "eolBaseOs": eol_val,
                "architecture": convert_to_safe_string(image.get("architecture")),
                "created": convert_epoch_ms_to_iso(image.get("created")),
                "lastScanned": convert_epoch_ms_to_iso(image.get("lastScanned")),
                "scanTypes": image.get("scanTypes") or [],
                "source": image.get("source") or [],
                "repositories": extract_image_repositories(image),
                "riskScore": image.get("riskScore"),
                "maxQdsScore": image.get("maxQdsScore"),
                "qdsSeverity": convert_to_safe_string(image.get("qdsSeverity")),
                "containerCount": container_counts.get(image_sha, "N/A") if not skip_containers else "N/A",
                "softwareCount": len(image.get("softwares") or []),
            }
        ...
        f.write(f'  "eolImages": {total_eol}\n')
        f.write('}\n')

    os.replace(temp_path, output_path)
    logger.info(f"  JSON: {output_path}")
    return total_images, total_eol
...
def main():
    ...
    # ── Phase 1: Fetch images (saved to disk as page files) ──
    if checkpoint.is_phase_complete("images"):
        ...
        api_client.fetch_all_pages(image_url, pages_dir, "img", args.limit)
        checkpoint.mark_complete("images")
    image_count = count_images_from_pages(pages_dir, "img")
    logger.info(f"  Total: {image_count} images")
    if image_count == 0: logger.warning("  WARNING: 0 images returned.")
    ...
    if checkpoint.is_phase_complete("eol"):
        ...
        api_client.fetch_all_pages(eol_url, pages_dir, "eol", args.limit)
        # Extract SHAs from EOL page files (streaming — not loaded all at once)
        eol_image_shas = set()
        for img in iterate_images_from_pages(pages_dir, "eol"):
            if img.get("sha"):
                eol_image_shas.add(img["sha"])
    ...
    if args.skip_containers:
        ...
        # Collect only SHAs from pages (not full images) — lightweight
        unique_shas = collect_shas_from_pages(pages_dir, "img")
    ...
    # ── Phase 4: Generate reports (STREAMING — constant memory) ──
    # Instead of loading all images into memory, we stream through
    # the page files one at a time. Memory stays at ~45 MB regardless
    # of whether you have 76 images or 10,000.
    logger.info("\n[Phase 4] Reports (streaming)...")
    csv_rows, total_images, total_eol = generate_csv_report_streaming(
        pages_dir, "img", eol_image_shas, container_counts, args.skip_containers,
        os.path.join(output_dir, "qualys_cs_unified_report.csv"), logger)
    generate_json_report_streaming(
        pages_dir, "img", eol_image_shas, container_counts, args.skip_containers,
        os.path.join(output_dir, "images_full_report.json"), logger)
    ...
    logger.info(f"  Images: {total_images} | EOL: {total_eol}")
